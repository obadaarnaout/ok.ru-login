...
class OdnoklassnikiSDK{
    ...
    private static $app_id = ""; // Application ID
    private static $app_public_key = ""; // Application Public Key
    private static $app_secret_key = ""; // Application Secret Key
    private static $redirect_url = ""; // Redirect Url
    ...
    public static function changeCodeToTokenTest($code){
        $curl = curl_init(self::$TOKEN_SERVICE_ADDRESS);
        curl_setopt($curl, CURLOPT_POST, true);
        curl_setopt($curl, CURLOPT_POSTFIELDS, 'code=' . $code . '&redirect_uri=' . self::$redirect_url . '&grant_type=authorization_code&client_id=' . self::$app_id . '&client_secret=' . self::$app_secret_key);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
